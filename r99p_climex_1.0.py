...
"""
Climate index: R99p
Definition: [Annual total PRCP when PR > 99th percentile] Let RRwj be the daily precipitation amount on a wet day
             w (RR ≥ 1.0mm) in period i and let RRwn99 be the 99th percentile of precipitation on wet days
...
for filename in glob.glob("*_RR_d.csv"):
    ...
    df_output_out_of_base = pd.DataFrame({"Date": dates, "r99p (mm)": r_lst_final})
    ...
    df_output_base_period = pd.DataFrame({"Date": base_year_lst, "r99p (mm)": index_base_list})
